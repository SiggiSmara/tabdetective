...
def lorem_paragraphs_wrapped(
        ...
        width:int = 80, 
        paragraph_sep:str ="\n\n", 
        paragraph_prefix:str="")->str:
    """Generate lorem ipsum paragraphs and wordwrap them"""
    return paragraph_sep.join([paragraph_prefix + force_wordwrap(lorem.paragraph(), width) for x in range(num_paragraphs)])
...
def testcase_02(table:str)->str:
    num_paragraphs_before = random.randint(1, 3)
    num_paragraphs_after = random.randint(1, 3)
    paragraphs_before = lorem_paragraphs_wrapped(num_paragraphs_before, paragraph_sep="\n", paragraph_prefix="    ")  
    paragraphs_after = lorem_paragraphs_wrapped(num_paragraphs_after, paragraph_sep="\n", paragraph_prefix="    ")
